...
// Detection LED: on when any tracked class is seen, off after no detection for kNoDetectionResetMs.
#ifndef DETECTION_LED_PIN
#ifdef LED_BUILTIN
#define DETECTION_LED_PIN LED_BUILTIN
#else
#define DETECTION_LED_PIN 21
#endif
#endif
...
void setup() {
    ...
    // Detection LED: reflects current detection state (on when detections, off when idle).
...
// Try 115200 first (GV2 default); if no reply, change to 9600 and reflash.
static const uint32_t kTestBaud = 115200;
...
void setup() {
    ...
    Serial1.begin(kTestBaud, SERIAL_8N1, 43, 44);
}
