...
- See [[colab_crash_prevention]], [[swift_yolo_colab_session_2026_02_06]], [[swift_yolo_colab_training_options_plan]].

## Current problems / risks

- **Colab disconnects:** Colab sessions disconnect; need to solve (e.g. Option B: train/valid on Drive, resume from checkpoint; see [[colab_crash_prevention]]).
...
- **Colab data source:** Test whether training with dataset via Roboflow API vs dataset unzipped in Colab makes a difference (OOM, credits, stability); add finding to [[colab_crash_prevention]] and here. **Summary:** OOM is mitigated by Option B (train/valid on Drive, no large unzip); unzip can trigger runtime kill; Roboflow API would stream per batch (see [[colab_crash_prevention]]).
