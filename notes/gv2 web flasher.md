**One-line purpose:** website for users to flash the model
**Short summary:** plan how to make website were users can flash the gv2
**Agent:** first part is made by user, second is AI generated [to be verified]
**Index:** [[himax from pt to flash]]

---
1. user clicks 'deploy model' (as sensecraft)
2. browser shows serial port picker
3. user selects gv2
4. connects
5. send data using xmodem protocol: 
6. 

```sh
python3 xmodem/xmodem_send.py \
  --port=/dev/cu.<<COM_PORT_HIER>> \
  --baudrate=921600 \
  --protocol=xmodem \
  --file=we2_image_gen_local/output_case1_sec_wlcsp/output.img \
  --model="<<PATH_NAAR_full_integer_quant_vela.tflite>> 0xB7B000 0x00000"
```



```sh
python3 xmodem/xmodem_send.py --port=/dev/tty.usbmodem58FA1047631 --baudrate=921600 --protocol=xmodem --file=we2_image_gen_local/output_case1_sec_wlcsp/output.img --model="../EPII_CM55M_APP_S/app/scenario_app/tflm_yolo11_od/yolo11n_full_integer_quant_192_241219_batch_matmul_vela.tflite 0xB7B000 0x00000"
```
